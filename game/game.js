...
function purchaseCursor(button) {
    ...
    const hasBought = button.getAttribute('data-bought');

    if (cleanPoints >= baseCost) {
        cleanPoints -= baseCost;
        cleanPointsPerClick *= multiplier;

        button.setAttribute('data-bought', 'true');
...
function checkShopVisibility() {
    shopButtons.forEach(button => {
        ...
        if (cleanPoints >= threshold && hasBought === 'false') { 
...
function loadData() {
    ...
    shopButtons.forEach(button => {
        const upgradeId = button.getAttribute('data-upgrade-id');
        const hasBought = localStorage.getItem(upgradeId) === 'true';
        button.setAttribute('data-bought', hasBought);
        if (hasBought) {
            button.style.display = 'none';
        }
    });
